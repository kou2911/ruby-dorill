def register_item(registered_item)
  ...
  name = gets.chomp
  puts "販売価格を入力してください："
  affter_price = gets.to_i
  puts "仕入れ値を入力してください："
  before_price = gets.to_i
  line = "---------------------------"
  item ={name: name, affter_price: affter_price, before_price: before_price}
  # 入力された値（ハッシュオブジェクトで管理している値）を表示する
  puts "商品名 : #{item[:name]}\n#{line}"
  puts "販売価格 :#{item[:affter_price]} 円\n#{line}"
  puts "仕入れ値 : #{item[:before_price]}円\n#{line}"
